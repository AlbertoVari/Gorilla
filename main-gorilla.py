...
def torch2tvm_module(torch_module: torch.nn.Module, torch_inputs: Tuple[torch.Tensor, ...], target):
    ...
    input_names = []
    input_shapes = {}
    print("torch2tvm_module enter module")
    with torch.no_grad():
...
#        print("INPUT_SHAPES",input_shapes)
        relay_module, params = tvm.relay.frontend.from_onnx(onnx_model, input_shapes)
        relay_module = tvm.relay.transform.DynamicToStatic()(relay_module)
# OK original        relay_module, params = tvm.relay.frontend.from_onnx(onnx_model, input_shapes)
#        print("RELAY_MODULE",relay_module)
#        exit()
#    with tvm.relay.build_config(opt_level=1,required_pass=None, disabled_pass=None, trace=None):
#    with autotvm.apply_history_best(log_file):
    with tvm.transform.PassContext(opt_level=2):
        graph, tvm_module, params = tvm.relay.build_module.build(relay_module, target, params=params)
        print("graph",graph,"tvm_module",tvm_module,"params",params)
    return graph, tvm_module, params


def torch2executor(torch_module: torch.nn.Module, torch_inputs: Tuple[torch.Tensor, ...], target):
    print("CHECK", "torch2executor")
    prefix = f"mobilenet_tsm_tvm_{target}"
    lib_fname = f'{prefix}.tar'
    graph_fname = f'{prefix}.json'
    params_fname = f'{prefix}.params'
    print (lib_fname,graph_fname,params_fname )
...
def main():
    ...
    print(cap)
    #EV3 command
    # ev3_up="sshpass -p 'maker' ssh robot@192.168.1.9 'python3 right-up.py'"
    # ev3_up="sshpass -p 'maker' ssh robot@192.168.1.9 'python3 right-down.py'"
    idx_prev = 0

    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect('192.168.1.9', username='robot', password='maker')
...
#NEW AV
        if idx == 17 and idx != idx_prev :
#           os.system(ev3_up)
            stdin_, stdout_, stderr_ = client.exec_command("python3 right-arm.py")
            stdout_.channel.recv_exit_status()
        elif idx == 16 and idx != idx_prev :
#           os.system(ev3_down)
            stdin_, stdout_, stderr_ = client.exec_command("python3 left-arm.py")
            stdout_.channel.recv_exit_status()
        else :
            pass
